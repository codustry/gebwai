...
gj.widget.prototype.getConfig = function (clientConfig, type) {
  ...
  uiLibrary = clientConfig.hasOwnProperty("uiLibrary")
    ? clientConfig.uiLibrary
    : config.uiLibrary;
  if (gj[type].config[uiLibrary]) {
    $.extend(true, config, gj[type].config[uiLibrary]);
  }

  iconsLibrary = clientConfig.hasOwnProperty("iconsLibrary")
    ? clientConfig.iconsLibrary
    : config.iconsLibrary;
...
gj.documentManager = {
  ...
  subscribeForEvent: function (eventName, widgetId, callback) {
    if (
      ...
      gj.documentManager.events[eventName] = [
        { widgetId: widgetId, callback: callback },
      ];
...
gj.core = {
      ...
      if (/^\d+$/.test(value)) {
        ...
        for (i = 0; i < formatParts.length; i++) {
          if (["d", "dd"].indexOf(formatParts[i]) > -1) {
            ...
            month = gj.core.messages[locale || "en-us"].monthShortNames.indexOf(
              dateParts[i]
            );
          } else if ("mmmm" === formatParts[i]) {
            month = gj.core.messages[locale || "en-us"].monthNames.indexOf(
              dateParts[i]
            );
  ...
  formatDate: function (date, format, locale) {
    ...
    for (i = 0; i < formatParts.length; i++) {
      ...
      switch (formatParts[i]) {
        ...
        case "ddd":
          result +=
            gj.core.messages[locale || "en-us"].weekDaysShort[date.getDay()] +
            separator;
          break;
        case "dddd":
          result +=
            gj.core.messages[locale || "en-us"].weekDays[date.getDay()] +
            separator;
          break;
        case "m":
        ...
        case "mmm":
          result +=
            gj.core.messages[locale || "en-us"].monthShortNames[
              date.getMonth()
            ] + separator;
          break;
        case "mmmm":
          result +=
            gj.core.messages[locale || "en-us"].monthNames[date.getMonth()] +
            separator;
          break;
        case "yy":
  ...
  setChildPosition: function (mainEl, childEl) {
    ...
    if (
      mainElRect.top + mainElHeight + childElHeight > window.innerHeight &&
      mainElRect.top > childElHeight
    ) {
      childEl.style.top =
        Math.round(mainElRect.top + scrollY - childElHeight - 3) + "px";
    } else {
      childEl.style.top =
        Math.round(mainElRect.top + scrollY + mainElHeight + 3) + "px";
    }

    if (mainElRect.left + childElWidth > document.body.clientWidth) {
      childEl.style.left =
        Math.round(mainElRect.left + scrollX + mainElWidth - childElWidth) +
        "px";
    } else {
  ...
  height: function (el, margin) {
    ...
    if (style.boxSizing === "border-box") {
      // border-box include padding and border within the height
  ...
  width: function (el, margin) {
    ...
    if (style.boxSizing === "border-box") {
      // border-box include padding and border within the width
...
gj.dialog.methods = {
  ...
  initialize: function ($dialog) {
    ...
    if (data.modal) {
      $dialog.wrapAll(
        '<div data-role="modal" class="' + data.style.modal + '"/>'
      );
    }
  ...
  resizable: function ($dialog) {
    ...
    $dialog.append(
      $('<div class="gj-resizable-handle gj-resizable-n"></div>').draggable(
        $.extend(true, { horizontal: false }, config)
      )
    );
    $dialog.append(
      $('<div class="gj-resizable-handle gj-resizable-e"></div>').draggable(
        $.extend(true, { vertical: false }, config)
      )
    );
    $dialog.append(
  ...
  resize: function (e, newPosition) {
    ...
    if (
      height &&
      (!data.minHeight || height >= data.minHeight) &&
    ...
    if (
      width &&
      (!data.minWidth || width >= data.minWidth) &&
  ...
  open: function ($dialog, title) {
    ...
    if ($footer.length && $footer.outerHeight()) {
      $dialog
        ...
        .css("margin-bottom", $footer.outerHeight());
    }
    if (title !== undefined) {
...
gj.draggable.methods = {
  init: function (jsConfig) {
    ...
    $handleEl.on("touchstart mousedown", function (e) {
      ...
      gj.documentManager.subscribeForEvent(
        "touchmove",
        $dragEl.data("guid"),
        gj.draggable.methods.createMoveHandler($dragEl, $handleEl, data)
      );
      gj.documentManager.subscribeForEvent(
        "mousemove",
        $dragEl.data("guid"),
  ...
  createUpHandler: function ($dragEl) {
    return function (e) {
      if ($dragEl.attr("draggable-dragging") === "true") {
        ...
        gj.documentManager.unsubscribeForEvent(
          "mousemove",
          $dragEl.data("guid")
        );
        gj.documentManager.unsubscribeForEvent(
  ...
  move: function (dragEl, data, offsetX, offsetY, mouseX, mouseY) {
    ...
    if (data.containment) {
      ...
      maxTop =
        contPosition.top +
        gj.core.height(data.containment) -
        gj.core.height(dragEl);
      maxLeft =
        contPosition.left +
        gj.core.width(data.containment) -
    ...
    if (
      false !==
      gj.draggable.events.drag($(dragEl), newLeft, newTop, mouseX, mouseY)
    ) {
...
gj.grid.events = {
  ...
  cellDataBound: function ($grid, $displayEl, id, column, record) {
    return $grid.triggerHandler("cellDataBound", [
      $displayEl,
      id,
      column,
      record,
...
gj.grid.methods = {
  ...
  initialize: function ($grid) {
    ...
    if ("checkbox" === data.selectionMethod) {
      data.columns.splice(
        gj.grid.methods.getColumnPositionNotInRole($grid),
        0,
        {
          title: "",
  ...
  renderHeader: function ($grid) {
    ...
    for (i = 0; i < columns.length; i += 1) {
      ...
      if (
        ...
        $title = $('<div data-role="title"/>').html(
          typeof columns[i].title === "undefined"
            ? columns[i].field
            : columns[i].title
        );
        $cell.append($title);
        if (columns[i].sortable) {
          ...
          $title.on(
            "click",
            gj.grid.methods.createSortHandler($grid, columns[i])
          );
        }
      }
      if (columns[i].hidden) {
  ...
  updateHeader: function ($grid) {
    ...
    if (sortBy) {
      position = gj.grid.methods.getColumnPosition(
        $grid.data("columns"),
        sortBy
      );
      if (position > -1) {
  ...
  startLoading: function ($grid) {
    ...
    $cover = $('<div data-role="loading-cover" />')
      .addClass(data.style.loadingCover)
      .css({
        width: width,
        height: height,
        top: top,
  ...
  renderRow: function ($grid, $row, record, position) {
    ...
    if (!$row || $row.length === 0) {
      ...
      mode = "update";
      $row
        .removeClass(data.style.content.rowSelected)
    ...
    for (i = 0; i < data.columns.length; i++) {
      if (mode === "update") {
        ...
        $cell = gj.grid.methods.renderCell(
          $grid,
          null,
          data.columns[i],
          record,
    ...
    if (column.events) {
      for (key in column.events) {
        if (column.events.hasOwnProperty(key)) {
          $cell.on(
            key,
            { id: id, field: column.field, record: record },
            gj.grid.methods.createCellEventHandler(column, column.events[key])
          );
        }
      }
    }
    ...
    if ("checkbox" === column.type && gj.checkbox) {
      if ("create" === mode) {
        $checkbox = $('<input type="checkbox" />')
          .val(id)
          .prop("checked", record[column.field] ? true : false);
      ...
      if ("create" === mode) {
        $displayEl.append(
          $("<span/>").addClass(column.icon).css({ cursor: "pointer" })
        );
  ...
  formatText: function (text, column) {
    if (text && ["date", "time", "datetime"].indexOf(column.type) > -1) {
      text = gj.core.formatDate(
        gj.core.parseDate(text, column.format),
        column.format
      );
    } else {
      text =
        typeof text === "undefined" || text === null ? "" : text.toString();
    }
  ...
  unselectRow: function ($grid, data, $row, id) {
    ...
    if ($row.attr("data-selected") === "true") {
      ...
      if ("checkbox" === data.selectionMethod) {
        $checkbox = $row.find(
          'td input[type="checkbox"][data-role="selectRow"]'
        );
        $checkbox.length &&
          $checkbox.prop("checked") &&
        ...
        if ("multiple" === data.selectionType) {
          $grid
            .find('thead input[data-role="selectAll"]')
            .prop("checked", false);
        }
      }
  ...
  getRowById: function ($grid, id) {
    ...
    if (position) {
      $result = $grid
        .children("tbody")
        .children('tr[data-position="' + position + '"]');
    }
    return $result;
  },
  ...
  setCellContent: function ($grid, id, field, value) {
    var column,
      $displayEl = gj.grid.methods.getCell($grid, id, field);
    if ($displayEl) {
      $displayEl.empty();
  ...
  clear: function ($grid, showNotFoundText) {
    ...
    data.records = [];
    gj.grid.methods.stopLoading($grid);
    gj.grid.methods.appendEmptyRow(
      $grid,
  ...
  filter: function ($grid) {
    ...
    if (data.params[data.paramNames.sortBy]) {
      column = gj.grid.methods.getColumnInfo(
        $grid,
        data.params[data.paramNames.sortBy]
      );
      records.sort(
        column.sortable.sorter
          ? column.sortable.sorter(column.direction, column)
          : gj.grid.methods.createDefaultSorter(column.direction, column.field)
      );
    }
    ...
    for (field in data.params) {
      if (data.params[field] && !data.paramNames[field]) {
        ...
        records = $.grep(records, function (record) {
          ...
          return column && typeof column.filter === "function"
            ? column.filter(value, searchStr)
            : value.toUpperCase().indexOf(searchStr.toUpperCase()) > -1;
        });
      }
    }
...
gj.grid.plugins.fixedHeader = {
  ...
  private: {
    ...
    refresh: function ($grid) {
      ...
      if (
        $grid.children("tbody").height() <
        gj.grid.plugins.fixedHeader.private.getRowsHeight($grid)
      ) {
        $tbody.css(
          "width",
          $thead.outerWidth() +
  ...
  private: {
    expandDetail: function ($grid, $cell, id) {
      var $contentRow = $cell.closest("tr"),
        ...
        $detailsCell = $(
          '<td colspan="' + gj.grid.methods.countVisibleColumns($grid) + '" />'
        ),
      ...
      $detailsRow.append(
        $detailsCell.append($detailsWrapper.append($contentRow.data("details")))
      );
      $detailsRow.insertAfter($contentRow);
      $cell
        .children('div[data-role="display"]')
        .empty()
        .append(data.icons.collapseRow);
    ...
    removeSelection: function ($grid, id) {
      var data = $grid.data();
      if (
        data.keepExpandedRows &&
        $.isArray(data.expandedRows) &&
      ...
      if (record && content) {
        $detailWrapper.html().replace(/\{(.+?)\}/g, function ($0, $1) {
          ...
          content = content.replace(
            $0,
            gj.grid.methods.formatText(record[$1], column)
          );
        });
    ...
    if (typeof data.detailTemplate !== "undefined") {
      column = {
        ...
        events: {
          click: function (e) {
            var $cell = $(this),
              methods = gj.grid.plugins.expandCollapseRows.private;
...
gj.grid.plugins.inlineEditing.private = {
  localization: function (data) {
    if (data.uiLibrary === "bootstrap") {
      data.inlineEditing.editButton =
        ...
        gj.grid.messages[data.locale].Edit +
        "</button>";
      data.inlineEditing.deleteButton =
        ...
        gj.grid.messages[data.locale].Delete +
        "</button>";
      data.inlineEditing.updateButton =
        ...
        gj.grid.messages[data.locale].Update +
        "</button>";
      data.inlineEditing.cancelButton =
        ...
        gj.grid.messages[data.locale].Cancel +
        "</button>";
    } else {
      data.inlineEditing.editButton =
  ...
  editMode: function ($grid, $cell, column, record) {
    ...
    if ($cell.attr("data-mode") !== "edit") {
      if (column.editor) {
        gj.grid.plugins.inlineEditing.private.updateOtherCells(
          $grid,
          column.mode
        );
        ...
        if ($editorField.length) {
          ...
          if (typeof column.editor === "function") {
            ...
            if ("checkbox" === column.type && gj.checkbox) {
              ...
              ("date" === column.type && gj.datepicker) ||
              ...
              if (data.uiLibrary === "materialdesign") {
                $editorField
                  ...
                  .css("font-size", $grid.css("font-size"));
              }
              $editorContainer.append($editorField);
            }
          }
          if (
            data.inlineEditing.mode !== "command" &&
        ...
        if (
          $editorField.prop("tagName").toUpperCase() === "INPUT" &&
    ...
    if (column.mode !== "editOnly") {
      if ($cell.attr("data-mode") === "edit") {
        ...
        if (cancel !== true && newValue !== record[column.field]) {
          record[column.field] =
            column.type === "date"
              ? gj.core.parseDate(newValue, column.format)
              : newValue;
          if (column.editField) {
          ...
          if (column.mode !== "editOnly") {
            gj.grid.methods.renderDisplayElement(
              ...
              column,
              record,
              gj.grid.methods.getId(record, $grid.data("primaryKey"), position),
              "update"
            );
          ...
          gj.grid.plugins.inlineEditing.events.cellDataChanged(
            $grid,
            $cell,
            column,
            record,
          ...
          gj.grid.plugins.inlineEditing.private.updateChanges(
            $grid,
            column,
            record,
            newValue
  ...
  updateChanges: function ($grid, column, sourceRecord, newValue) {
    ...
    if (data.primaryKey) {
      targetRecords = JSON.parse(
        sessionStorage.getItem("gj.grid." + data.guid)
      );
      if (targetRecords) {
...
gj.grid.plugins.inlineEditing.events = {
  ...
  cellDataChanged: function ($grid, $cell, column, record, oldValue, newValue) {
    $grid.triggerHandler("cellDataChanged", [
      $cell,
      column,
      record,
      oldValue,
...
gj.grid.plugins.inlineEditing.configure = function (
  $grid,
  fullConfig,
  clientConfig
) {
      ...
      if (column.editor) {
        if (column.mode === "editOnly") {
          ...
          $displayEl
            .parent("td")
            .on(
              data.inlineEditing.mode === "dblclick" ? "dblclick" : "click",
              function () {
                gj.grid.plugins.inlineEditing.private.editMode(
                  $grid,
                  $displayEl.parent(),
...
gj.grid.plugins.optimisticPersistence = {
  ...
  private: {
    ...
    saveParams: function ($grid) {
      ...
      if (data.optimisticPersistence.sessionStorage) {
        ...
        storage = $.extend(
          true,
          JSON.parse(sessionStorage.getItem("gj.grid." + data.guid)),
          storage
        );
      ...
      if (data.optimisticPersistence.localStorage) {
        ...
        storage = $.extend(
          true,
          JSON.parse(localStorage.getItem("gj.grid." + data.guid)),
          storage
        );
  ...
  configure: function ($grid, fullConfig, clientConfig) {
    if (fullConfig.guid) {
      if (
        fullConfig.optimisticPersistence.localStorage ||
  ...
  private: {
    init: function ($grid) {
      ...
      if (data.pager) {
        ...
        $leftPanel = $('<div data-role="display" />')
          .addClass(data.style.pager.panel)
          .css({ float: "left" });
        $rightPanel = $('<div data-role="display" />')
          .addClass(data.style.pager.panel)
          .css({ float: "right" });
      ...
      if (typeof data.pager.leftControls === "undefined") {
        data.pager.leftControls = [
          $(
            ...
            .attr("title", msg.FirstPageTooltip)
          ...
          $(
            ...
            .attr("title", msg.PreviousPageTooltip)
          ...
          $(
            ...
            .attr("title", msg.NextPageTooltip)
          ...
          $(
            ...
            .attr("title", msg.LastPageTooltip)
      ...
      if (typeof data.pager.rightControls === "undefined") {
        data.pager.rightControls = [
          ...
          $(
            ...
            .attr("title", msg.PreviousPageTooltip)
            .attr("data-role", "page-previous")
            .addClass(data.icons.first ? "gj-button-md-icon" : ""),
    ...
    reloadPager: function ($grid, totalRecords) {
      ...
      if (data.pager) {
        page =
          0 === totalRecords
            ? 0
            : parseInt(data.params[data.paramNames.page], 10);
        limit = parseInt(data.params[data.paramNames.limit], 10);
        lastPage = Math.ceil(totalRecords / limit);
        firstRecord = 0 === page ? 0 : limit * (page - 1) + 1;
        lastRecord =
          firstRecord + limit > totalRecords
            ? totalRecords
            : firstRecord + limit - 1;
      ...
      switch ($control.data("role")) {
        ...
        case "page-number":
          $control
            ...
            .on(
              "change",
              gj.grid.plugins.pagination.private.createChangePageHandler(
                $grid,
                page
              )
            );
    ...
    assignPageHandler: function ($grid, $control, newPage, disabled) {
      ...
      if (disabled) {
        $control
          .addClass(style.stateDisabled)
          .prop("disabled", true)
          .off("click");
      } else {
        $control
          .removeClass(style.stateDisabled)
          .prop("disabled", false)
          .off("click")
          .on("click", function () {
    ...
    getAll: function (includeAllRecords) {
      ...
      if ($.isArray(data.dataSource)) {
        if (includeAllRecords) {
          ...
          data.params[data.paramNames.limit] &&
...
gj.grid.plugins.responsiveDesign = {
    ...
    updateDetails: function ($grid) {
      ...
      for (i = 0; i < rows.length; i++) {
        ...
        for (j = 0; j < data.columns.length; j++) {
          ...
          $placeholder =
            details && details.find('div[data-id="' + column.field + '"]');
          if (data.columns[j].hidden) {
            ...
            if (!$placeholder || !$placeholder.length) {
              $placeholder = $('<div data-id="' + column.field + '"/>').html(
                tmp
              );
  ...
  configure: function ($grid, fullConfig, clientConfig) {
    ...
    if (
      fullConfig.showHiddenColumnsAsDetails &&
      gj.grid.plugins.expandCollapseRows
    ) {
      $grid.on("dataBound", function () {
...
gj.grid.plugins.toolbar = {
  ...
  private: {
    init: function ($grid) {
      ...
      if (
        typeof data.toolbarTemplate !== "undefined" ||
    ...
    init: function ($grid, config) {
      ...
      if ($columns.length) {
        for (i = 0; i < $columns.length - 1; i++) {
          ...
          $resizer = $('<span class="gj-grid-column-resizer" />').css(
            "margin-right",
            "-" + marginRight + "px"
          );
          $resizer.draggable({
  ...
  private: {
    init: function ($grid) {
      ...
      if ($grid.data("rowReorderColumn")) {
        columnPosition = gj.grid.methods.getColumnPosition(
          $grid.data("columns"),
          $grid.data("rowReorderColumn")
        );
      }
      for (i = 0; i < $rows.length; i++) {
        $row = $($rows[i]);
    ...
    createRowMouseDownHandler: function ($grid, $trSource) {
      return function (e) {
        ...
        $dragEl
          .find(
            'tbody tr:not([data-position="' + $trSource.data("position") + '"])'
          )
          .remove();
        ...
        $trSource.siblings('tr[data-role="row"]').each(function () {
          ...
          if ($trTarget.droppable("isOver", mousePosition)) {
            ...
            data.records.splice(
              targetPosition - 1,
              0,
              data.records.splice(sourcePosition - 1, 1)[0]
            );
            ...
            if (data.orderNumberField) {
              ...
              for (i = 0; i < $rows.length; i++) {
                ...
                record = gj.grid.methods.getByPosition(
                  $grid,
                  $row.attr("data-position")
                );
                $grid.setCellContent(
      ...
      if (records.length) {
        for (i = 0; i < columns.length; i++) {
          if (gj.grid.plugins.export.public.isColumnApplicable(columns[i])) {
            line +=
              '"' +
              (columns[i].title || columns[i].field).replace(/<[^>]+>/g, " ") +
              '",';
          }
        }
    ...
    downloadCSV: function (filename, includeAllRecords) {
      ...
      if (window.navigator.userAgent.indexOf("Edge") > -1) {
        link.href = URL.createObjectURL(
          new Blob([this.getCSV(includeAllRecords)], {
  ...
  private: {
    init: function ($grid) {
      ...
      for (i = 0; i < $cells.length; i++) {
        ...
        $cell.on(
          "mousedown",
          gj.grid.plugins.columnReorder.private.createMouseDownHandler(
            $grid,
            $cell
          )
        );
        $cell.on(
          "mousemove",
          gj.grid.plugins.columnReorder.private.createMouseMoveHandler(
            $grid,
            $cell
          )
        );
        $cell.on(
          "mouseup",
          gj.grid.plugins.columnReorder.private.createMouseUpHandler(
            $grid,
            $cell
          )
        );
    ...
    createMouseMoveHandler: function ($grid, $thSource) {
      return function (e) {
        ...
        if ($grid.data("dragReady")) {
          ...
          $dragEl.attr("data-role", "draggable-clone").css("cursor", "move");
          $dragEl
            .find("thead tr th:eq(" + srcIndex + ")")
            .siblings()
            .remove();
          $dragEl.find('tbody tr[data-role != "row"]').remove();
          $dragEl
            .find("tbody tr td:nth-child(" + (srcIndex + 1) + ")")
            .siblings()
            .remove();
    ...
    createDragStopHandler: function ($grid, $thSource) {
      return function (e, mousePosition) {
        ...
        $thSource.siblings("th").each(function () {
          var $thTarget = $(this),
            data = $grid.data(),
            targetPosition = gj.grid.methods.getColumnPosition(
              data.columns,
              $thTarget.data("field")
            ),
            sourcePosition = gj.grid.methods.getColumnPosition(
              data.columns,
              $thSource.data("field")
            );
          ...
          if ($thTarget.droppable("isOver", mousePosition)) {
            ...
            gj.grid.plugins.columnReorder.private.moveRowCells(
              $grid,
              sourcePosition,
              targetPosition
            );
            data.columns.splice(
              targetPosition,
              0,
              data.columns.splice(sourcePosition, 1)[0]
            );
          }
    ...
    moveRowCells: function ($grid, sourcePosition, targetPosition) {
      ...
      for (i = 0; i < $rows.length; i++) {
        ...
        if (targetPosition < sourcePosition) {
          $row
            .find("td:eq(" + targetPosition + ")")
            .before($row.find("td:eq(" + sourcePosition + ")"));
        } else {
          $row
            .find("td:eq(" + targetPosition + ")")
            .after($row.find("td:eq(" + sourcePosition + ")"));
        }
      }
    },
    ...
    init: function ($grid) {
      ...
      $grid.on("rowDataBound", function (e, $row, id, record) {
        if (
          ...
          $expandCollapseCell.append(
            '<div data-role="display">' + data.icons.collapseGroup + "</div>"
          );
          $expandCollapseCell.on(
            "click",
            gj.grid.plugins.grouping.private.createExpandCollapseHandler(data)
          );
...
gj.tree.methods = {
  ...
  appendNode: function ($tree, $parent, nodeData, level, position) {
    var i,
      ...
      data = $tree.data(),
      id = data.primaryKey
        ? nodeData[data.primaryKey]
        : nodeData[data.autoGenFieldName];
    ($node = $('<li data-id="' + id + '" data-role="node" />').addClass(
      data.style.item
    )),
      ...
      ($display = $(
        '<span data-role="display">' + nodeData[data.textField] + "</span>"
      )),
      (hasChildren =
        typeof nodeData[data.hasChildrenField] !== "undefined" &&
        nodeData[data.hasChildrenField].toString().toLowerCase() === "true"),
      (disabled =
        typeof nodeData[data.disabledField] !== "undefined" &&
        nodeData[data.disabledField].toString().toLowerCase() === "true");
    ...
    if (data.imageCssClassField && nodeData[data.imageCssClassField]) {
      $span = $(
        '<span data-role="image"><span class="' +
          nodeData[data.imageCssClassField] +
          '"></span></span>'
      );
      ...
      $span = $(
        '<span data-role="image">' + nodeData[data.imageHtmlField] + "</span>"
      );
      $span.insertBefore($display);
    }

    if (
      (nodeData[data.childrenField] && nodeData[data.childrenField].length) ||
      hasChildren
    ) {
      ...
      if (nodeData[data.childrenField] && nodeData[data.childrenField].length) {
        for (i = 0; i < nodeData[data.childrenField].length; i++) {
          gj.tree.methods.appendNode(
            $tree,
            $newParent,
            nodeData[data.childrenField][i],
            level + 1
          );
        }
      }
      ...
      data.style.leafIcon
        ? $expander.addClass(data.style.leafIcon)
        : $expander.html("&nbsp;");
    }
  ...
  expand: function ($tree, $node, cascade) {
    ...
    if (
      gj.tree.events.expand($tree, $node, id) !== false &&
      $list &&
      $list.length
    ) {
      $list.show();
  ...
  collapse: function ($tree, $node, cascade) {
    var $children,
      ...
      $list = $node.children("ul");
    if (
      gj.tree.events.collapse($tree, $node, id) !== false &&
      $list &&
      $list.length
    ) {
      $list.hide();
  ...
  getDataById: function ($tree, id, records) {
    ...
    for (i = 0; i < records.length; i++) {
      if (data.primaryKey && records[i][data.primaryKey] == id) {
        ...
        result = gj.tree.methods.getDataById(
          $tree,
          id,
          records[i][data.childrenField]
        );
  ...
  getDataByText: function ($tree, text, records) {
    ...
    for (i = 0; i < records.length; i++) {
      if (text === records[i][data.textField]) {
        ...
        result = gj.tree.methods.getDataByText(
          $tree,
          text,
          records[i][data.childrenField]
        );
  ...
  getNodeByText: function ($list, text) {
    ...
    if ($nodes && $nodes.length) {
      for (i = 0; i < $nodes.length; i++) {
        ...
        if (
          text ===
          $node.find('>[data-role="wrapper"]>[data-role="display"]').text()
        ) {
          $result = $node;
          break;
        } else if ($node.has("ul")) {
  ...
  pathFinder: function (data, list, id, parents) {
    ...
    for (i = 0; i < list.length; i++) {
      if (list[i].id == id) {
        ...
        gj.tree.methods.pathFinder(
          data,
          list[i][data.childrenField],
          id,
          parents
        )
      ) {
...
gj.tree.plugins.checkboxes = {
  ...
  private: {
    ...
    nodeDataBound: function ($tree, $node, id, record) {
      ...
      if (
        $node.find('> [data-role="wrapper"] > [data-role="checkbox"]')
          .length === 0
      ) {
        data = $tree.data();
        $expander = $node.find(
          '> [data-role="wrapper"] > [data-role="expander"]'
        );
        $checkbox = $('<input type="checkbox"/>');
        $wrapper = $('<span data-role="checkbox"></span>').append($checkbox);
        disabled =
          typeof record[data.disabledField] !== "undefined" &&
    ...
    updateParentState: function ($node, state) {
      ...
      if ($parentNode.length === 1) {
        ...
        gj.tree.plugins.checkboxes.private.updateParentState(
          $parentNode,
          $parentCheckbox.checkbox("state")
        );
      }
    },

    updateChildrenState: function ($node, state) {
      var $childrenCheckboxes = $node.find(
    ...
    getCheckedNodes: function () {
      var result = [],
        checkboxes = this.find(
          'li [data-role="checkbox"] input[type="checkbox"]'
        );
  ...
  configure: function ($tree) {
    if ($tree.data("checkboxes") && gj.checkbox) {
      ...
      $tree.on("nodeDataBound", function (e, $node, id, record) {
        gj.tree.plugins.checkboxes.private.nodeDataBound(
          $tree,
          $node,
          id,
          record
  ...
  private: {
    nodeDataBound: function ($tree, $node) {
      ...
      if ($wrapper.length && $display.length) {
        $display.on(
          "mousedown",
          gj.tree.plugins.dragAndDrop.private.createNodeMouseDownHandler($tree)
        );
        $display.on(
          "mousemove",
        ...
        $display.on(
          "mouseup",
          gj.tree.plugins.dragAndDrop.private.createNodeMouseUpHandler($tree)
        );
      }
    },
    ...
    createNodeMouseMoveHandler: function ($tree, $node, $display) {
      return function (e) {
        if ($tree.data("dragReady")) {
          ...
          offsetTop -=
            parseInt($wrapper.css("border-top-width")) +
          ...
          gj.tree.plugins.dragAndDrop.private
            .getTargetDisplays($tree, $node, $display)
            .each(function () {
              var $dropEl = $(this);
              if ($dropEl.attr("data-droppable") === "true") {
                $dropEl.droppable("destroy");
    ...
    getTargetDisplays: function ($tree, $node, $display) {
      return $tree
        ...
        .not($display)
        .not($node.find('[data-role="display"]'));
    },

    getTargetWrappers: function ($tree, $node) {
      return $tree
        .find('[data-role="wrapper"]')
        .not($node.find('[data-role="wrapper"]'));
    },

    createDragHandler: function ($tree, $node, $display) {
      var $displays = gj.tree.plugins.dragAndDrop.private.getTargetDisplays(
          $tree,
          $node,
          $display
        ),
        $wrappers = gj.tree.plugins.dragAndDrop.private.getTargetWrappers(
          $tree,
          $node
        ),
        data = $tree.data();
      return function (e, offset, mousePosition) {
        ...
        $displays.each(function () {
          ...
          if ($targetDisplay.droppable("isOver", mousePosition)) {
            ...
            data.style.dropAsChildIcon
              ? $indicator.addClass(data.style.dropAsChildIcon)
              : $indicator.text("+");
            success = true;
            return false;
          } else {
            $dragEl
              .find('[data-role="indicator"]')
              .removeClass(data.style.dropAsChildIcon)
              .empty();
          }
        });
        $wrappers.each(function () {
          ...
          if (!success && $wrapper.droppable("isOver", mousePosition)) {
            ...
            if (mousePosition.y < middle) {
              $wrapper
                .addClass(data.style.dropAbove)
                .removeClass(data.style.dropBelow);
            } else {
              $wrapper
                .addClass(data.style.dropBelow)
        ...
        $displays.each(function () {
          ...
          if ($targetDisplay.droppable("isOver", mousePosition)) {
            ...
            if (
              ...
              gj.tree.plugins.dragAndDrop.private.refresh(
                $tree,
                $sourceNode,
                $targetNode,
                $sourceParentNode
        ...
        if (!success) {
          $wrappers.each(function () {
            ...
            if ($targetWrapper.droppable("isOver", mousePosition)) {
              ...
              prepend =
                mousePosition.y <
                $targetWrapper.position().top +
                  $targetWrapper.outerHeight() / 2;
              sourceNodeId = $sourceNode.data("id");
              orderNumber =
                $targetNode.prevAll('li:not([data-id="' + sourceNodeId + '"])')
                  .length + (prepend ? 1 : 2);
              if (
                gj.tree.plugins.dragAndDrop.events.nodeDrop(
                  $tree,
                  sourceNodeId,
                  $targetNode.parent("ul").parent("li").data("id"),
                  orderNumber
                ) !== false
              ) {
                //BEGIN: Change node position inside the backend data
                ...
                $tree
                  .getDataById($targetNode.parent().data("id"))
                  [data.childrenField].splice(
                    $targetNode.index() + (prepend ? 0 : 1),
                    0,
                    record
                  );
                //END
...
gj.tree.plugins.lazyLoading = {
    ...
    expand: function ($tree, $node, id) {
      ...
      if (!$children || !$children.length) {
        if (typeof data.dataSource === "string") {
          ...
          $tree.xhr = $.ajax(ajaxOptions)
            .done(
              gj.tree.plugins.lazyLoading.private.createDoneHandler(
                $tree,
                $node
              )
            )
            .fail($tree.createErrorHandler());
        }
      }
    },
  },
  ...
  configure: function ($tree, fullConfig, clientConfig) {
    if (clientConfig.lazyLoading) {
      $tree.on("nodeDataBound", function (e, $node, id, record) {
        gj.tree.plugins.lazyLoading.private.nodeDataBound(
          $tree,
          $node,
          id,
          record
...
gj.checkbox.methods = {
  ...
  initialize: function ($chkb) {
    ...
    if (data.style.wrapperCssClass) {
      $wrapper = $(
        '<label class="' +
          data.style.wrapperCssClass +
          " " +
          data.style.iconsCssClass +
          '"></label>'
      );
      if ($chkb.attr("id")) {
...
gj.editor.methods = {
  ...
  initialize: function ($editor) {
    ...
    $body.on("keydown", function (e) {
      ...
      if (
        gj.editor.events.changing($editor) === false &&
        key !== 8 &&
        key !== 46
      ) {
        e.preventDefault();
...
gj.dropdown.methods = {
  ...
  initialize: function ($dropdown) {
    var $item,
      ...
      $presenter = $(
        '<button role="presenter" type="button"></button>'
      ).addClass(data.style.presenter),
      $list = $('<ul role="list" class="' + data.style.list + '"></ul>').attr(
        "guid",
        $dropdown.attr("data-guid")
      );
  ...
  setListPosition: function (presenter, list, data) {
    ...
    if (data.maxHeight === "auto") {
      if (mainElRect.top < listElRect.top) {
        // The list is located below the main element
        if (
          mainElRect.top + listHeight + presenterHeight >
          window.innerHeight
        ) {
          newHeight = window.innerHeight - mainElRect.top - presenterHeight - 3;
        }
      } else {
        // The list is located above the main element
        if (mainElRect.top - listHeight - 3 > 0) {
          list.style.top =
            Math.round(mainElRect.top + scrollY - listHeight - 3) + "px";
        } else {
  ...
  render: function ($dropdown, response) {
    var selections = [],
      ...
      $list = $("body").children(
        '[role="list"][guid="' + $dropdown.attr("data-guid") + '"]'
      ),
      ...
      $.each(response, function () {
        var value = this[data.valueField],
          ...
          selected =
            ...
            this[data.selectedField].toString().toLowerCase() === "true",
          i,
          $item;
...
gj.datepicker.methods = {
  ...
  initialize: function ($datepicker, data) {
    ...
    if (data.keyboardNavigation) {
      $(document).on(
        "keydown",
        gj.datepicker.methods.createKeyDownHandler($datepicker, $calendar, data)
      );
    }
  },
  ...
  createCalendar: function ($datepicker, data) {
    var date,
      ...
      $btnCancel,
      $btnOk,
      $calendar = $('<div role="calendar" type="month"/>')
        .addClass(data.style.calendar)
    ...
    if (data.footer) {
      ...
      $btnCancel = $(
        '<button class="' +
          data.style.button +
          '">' +
          gj.core.messages[data.locale].cancel +
          "</button>"
      );
      ...
      $btnOk.on("click", function () {
        var date,
          dayArr,
          dayStr = $calendar.attr("selectedDay");
        if (dayStr) {
          dayArr = dayStr.split("-");
          date = new Date(
            dayArr[0],
            dayArr[1],
            dayArr[2],
            $calendar.attr("hour") || 0,
            $calendar.attr("minute") || 0
          );
  ...
  updateHeader: function ($calendar, data, date) {
    ...
    $calendar
      ...
      .removeClass("selected")
      .html(gj.core.formatDate(date, "MM", data.locale));
  },
  ...
  createNavigation: function ($datepicker, $body, $table, data) {
    ...
    $navigator.append(
      $("<div>" + data.icons.previousMonth + "</div>").on(
        "click",
    ...
    $navigator.append(
      $("<div>" + data.icons.nextMonth + "</div>").on(
        "click",
    ...
    for (
      i = data.weekStartDay;
      i < gj.core.messages[data.locale].weekDaysMin.length;
      i++
    ) {
      $row.append(
        "<th><div>" +
  ...
  renderMonth: function ($datepicker, $calendar, data) {
    ...
    period = period
      .replace("mmmm", gj.core.messages[data.locale].monthNames[month])
      .replace("yyyy", year);
    ...
    firstDayPosition =
      (new Date(year, month, 1).getDay() + 7 - data.weekStartDay) % 7;
    ...
    for (i = 1; i <= firstDayPosition; i++) {
      ...
      if (data.showOtherMonths) {
        ...
        $cell.append($day);
        if (
          data.selectOtherMonths &&
          gj.datepicker.methods.isSelectable(data, date)
        ) {
          $cell
            .addClass("gj-cursor-pointer")
            .attr("day", day)
            .attr("month", prevMonth.month)
            .attr("year", prevMonth.year);
          $day.on(
            "click",
    ...
    for (i = 1; i <= total; i++) {
      ...
      $cell = $(
        '<td day="' + i + '" month="' + month + '" year="' + year + '" />'
      );
      if (
      ...
      if (data.showOtherMonths) {
        ...
        if (
          ...
          $cell
            .addClass("gj-cursor-pointer")
            .attr("day", i)
            .attr("month", nextMonth.month)
            .attr("year", nextMonth.year);
          $day.on(
            "click",
            gj.datepicker.methods.dayClickHandler(
              $datepicker,
  ...
  renderYear: function ($datepicker, $calendar, data) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      for (m = i * 4; m <= i * 4 + 3; m++) {
        $month = $(
          "<div>" + gj.core.messages[data.locale].monthShortNames[m] + "</div>"
        );
        $month.on(
          "click",
          gj.datepicker.methods.selectMonth($datepicker, $calendar, data, m)
        );
  ...
  renderDecade: function ($datepicker, $calendar, data) {
    ...
    for (i = decade - 1; i <= decade + 10; i += 4) {
      ...
      for (y = i; y <= i + 3; y++) {
        ...
        $year.on(
          "click",
          gj.datepicker.methods.selectYear($datepicker, $calendar, data, y)
        );
  ...
  renderCentury: function ($datepicker, $calendar, data) {
    ...
    for (i = century - 10; i < century + 100; i += 40) {
      ...
      for (d = i; d <= i + 30; d += 10) {
        ...
        $decade.on(
          "click",
          gj.datepicker.methods.selectDecade($datepicker, $calendar, data, d)
        );
  ...
  isSelectable: function (data, date) {
    ...
    if (result) {
      if (data.disableDates) {
        if ($.isArray(data.disableDates)) {
          for (i = 0; i < data.disableDates.length; i++) {
            if (
              data.disableDates[i] instanceof Date &&
              data.disableDates[i].getTime() === date.getTime()
            ) {
              result = false;
            } else if (
              typeof data.disableDates[i] === "string" &&
              gj.core
                .parseDate(data.disableDates[i], data.format, data.locale)
      ...
      if (
        $.isArray(data.disableDaysOfWeek) &&
  ...
  selectDay: function ($datepicker, $calendar, data, date) {
    ...
    $calendar
      .find('tbody td[day="' + day + '"][month="' + month + '"]')
      .addClass("selected");
    gj.datepicker.methods.updateHeader($calendar, data, date);
...
gj.timepicker.methods = {
  ...
  initMouse: function ($body, $input, $picker, data) {
    ...
    $body.on(
      "mousedown",
      gj.timepicker.methods.mouseDownHandler($input, $picker)
    );
    $body.on(
      "mousemove",
      gj.timepicker.methods.mouseMoveHandler($input, $picker, data)
    );
    $body.on(
      "mouseup",
      gj.timepicker.methods.mouseUpHandler($input, $picker, data)
    );
  },
  ...
  createPicker: function ($timepicker) {
    var date,
      ...
      $clock = $('<div role="picker" />')
        .addClass(data.style.clock)
        .attr("guid", $timepicker.attr("data-guid")),
      $hour = $('<div role="hour" />'),
    ...
    if (data.header) {
      ...
      if (data.mode === "ampm") {
        $mode.append(
          $(
            '<span role="am">' + gj.core.messages[data.locale].am + "</span>"
          ).on("click", function () {
        ...
        $mode.append(
          $(
            '<span role="pm">' + gj.core.messages[data.locale].pm + "</span>"
          ).on("click", function () {
            var hour = gj.timepicker.methods.getHour($clock);
            $clock.attr("mode", "pm");
            $(this).addClass("selected");
  ...
  updateArrow: function (e, $timepicker, $clock, data) {
    ...
    if (data.dialMode == "hours") {
      value = gj.timepicker.methods.getPointerValue(
        mouseX - scrollX - rect.left,
        mouseY - scrollY - rect.top,
        data.mode
      );
      $clock.attr(
        "hour",
        data.mode === "ampm" && $clock.attr("mode") === "pm" && value < 12
          ? value + 12
          : value
      );
  ...
  update: function ($timepicker, $clock, data) {
    ...
    if (data.dialMode == "hours") {
      ...
      $arrow.css(
        "transform",
        "rotate(" + (minute * 6 - 90).toString() + "deg)"
      );
    }
    $arrow.show();
    ...
    // update the numbers
    visualHour =
      data.mode === "ampm" && hour > 12 ? hour - 12 : hour == 0 ? 12 : hour;
  ...
  open: function ($timepicker) {
    var time,
      ...
      data = $timepicker.data(),
      $clock = $("body").find(
        '[role="picker"][guid="' + $timepicker.attr("data-guid") + '"]'
      );
...
gj.datetimepicker.methods = {
  ...
  initialize: function ($datetimepicker) {
    ...
    date = data.value
      ? gj.core.parseDate(data.value, data.format, data.locale)
      : new Date();
    ...
    $date.on(
      "click",
      gj.datetimepicker.methods.createShowDateHandler(
        $datetimepicker,
        $picker,
        data
      )
    ...
    $calendarMode = $(
      '<i class="gj-icon selected" role="calendarMode">event</i>'
    );
    $calendarMode.on(
      "click",
      gj.datetimepicker.methods.createShowDateHandler(
    ...
    $time.append(
      $('<div role="hour" />')
        ...
        .html(gj.core.formatDate(new Date(), "HH", data.locale))
    );
    $time.append(":");
    $time.append(
      $('<div role="minute" />')
        ...
        .html(gj.core.formatDate(new Date(), "MM", data.locale))
    );
    $switch.append($time);
  ...
  createShowDateHandler: function ($datetimepicker, $picker, data) {
    return function (e) {
      ...
      gj.datepicker.methods.renderMonth(
        $datetimepicker,
        $picker,
        data.datepicker
      );
  ...
  createShowHourHandler: function ($datetimepicker, $picker, data) {
    return function () {
      ...
      gj.timepicker.methods.initMouse(
        $picker.children('[role="body"]'),
        $datetimepicker,
        $picker,
        data.timepicker
      );
      gj.timepicker.methods.renderHours(
        $datetimepicker,
        $picker,
        data.timepicker
      );
  ...
  createShowMinuteHandler: function ($datetimepicker, $picker, data) {
    return function () {
      ...
      gj.timepicker.methods.renderMinutes(
        $datetimepicker,
        $picker,
        data.timepicker
      );
...
gj.slider.methods = {
  ...
  initialize: function ($slider, data) {
    ...
    handle.addEventListener(
      "mousedown",
      gj.slider.methods.createMouseDownHandler(handle, data)
    );
    gj.documentManager.subscribeForEvent(
      "mousemove",
      $slider.data("guid"),
      gj.slider.methods.createMouseMoveHandler(
        $slider,
        track,
        handle,
        progress,
    ...
    wrapper.addEventListener(
      "click",
      gj.slider.methods.createClickHandler($slider, track, handle, data)
    );
  },
...
gj.colorpicker.methods = {
  ...
  createPicker: function ($input, data) {
    var $picker = $('<div role="picker" />')
      .addClass(data.style.picker)
